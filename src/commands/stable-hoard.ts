...
export default {
  command: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option.setName("cfg").setDescription("The cfg to use").setRequired(false)
    ),
  ...
  async handler(
    ...
    var height = (interaction.options.get("height")?.value as string) ?? "512";
    ...
    const messageData = {
      ...
      files: [
        new AttachmentBuilder(await imageJoin(buff)).setName(`generation.png`),
      ],
      embeds: [
        {
