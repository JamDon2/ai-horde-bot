...
export default {
    command: new SlashCommandBuilder()
        ...
        .addStringOption((option) =>
            option.setName("name").setDescription("The name or ID of the team").setAutocomplete(true).setRequired(true)
        ),
    async commandHandler(
        ...
        const query = interaction.options.get("name", true).value?.toString().toLowerCase();
        ...
        const team = teams.find(t => t.name.toLowerCase() === query || t.id?.toLowerCase() === query)
